...
void sortPoints(point_t pts[], int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = (i+1); j < size; j++) {
            if (pts[j].x < pts[min_index].x) {
                temp = pts[min_index].x;
                pts[min_index].x = pts[j].x;
                pts[j].x = temp;

                temp = pts[min_index].y;
                pts[min_index].y = pts[j].y;
                pts[j].y = temp;
            }
            else if (pts[j].x == pts[min_index].x) {
                if (pts[j].y < pts[min_index].y) {
                    temp = pts[min_index].x;
                    pts[min_index].x = pts[j].x;
                    pts[j].x = temp;
...
int traceLines(point_t pts[], int size) {
    ...
    for (int i = 0; i < (size - 1); i++) {
        if (pts[i].x == pts[i+1].x) {
            sum += (pts[i+1].y - pts[i].y);
        }
        else if (pts[i].y == pts[i+1].y) {
            sum += (pts[i+1].x - pts[i].x);
        }
    }
